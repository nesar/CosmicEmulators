...
t =  XY[:,0,:].T     #  Should be the parameters
yW = np.loadtxt('Data/W_for2Eval.txt', dtype=float)  # 
              # Should be W
...
hmf_gp1 = gp1.predict( yW[0], t )
hmf_gp2 = gp1.predict( yW[1], t )



hmf_pred = np.array([hmf_gp1[0], hmf_gp2[0]])


hmfMock = np.loadtxt('Data/HMFTestData.txt')[5:]               
#Prediction = np.matmul(hmfMock, hmf_pred)
